...
/* =========================================================
   CUSTOMER ACCOUNTS
   Additive only — keeps guest checkout and existing orders.
   ========================================================= */
...
function customerAccountView(row) {
  if (!row) return null;
  return {
    id: row.id,
    name: row.name || '',
...
async function hashCustomerPassword(password, saltHex) {
  const enc = new TextEncoder();
...
async function customerSession(request, env) {
  ...
  const r = await env.DB.prepare(`
    ...
    LIMIT 1
  `).bind(token).first();
  return r ? { token, account: r } : null;
}

async function createCustomerSession(env, accountId) {
  const token = randomHex(32);
  await env.DB.prepare(`
    INSERT INTO customer_sessions (account_id, token, expires_at)
...
async function customerRegister(request, env) {
  ...
  if (!/^\d{10}$/.test(mobile)) return json({error:'Valid 10-digit mobile is required'},400);
  if (password.length < 8) return json({error:'Password must be at least 8 characters'},400);
  if (email && !/^\S+@\S+\.\S+$/.test(email)) return json({error:'Please enter a valid email'},400);
  if (pincode && !/^\d{6}$/.test(pincode)) return json({error:'Pincode must be 6 digits'},400);

  const existing = await env.DB.prepare(`SELECT id FROM customer_accounts WHERE mobile = ? LIMIT 1`).bind(mobile).first();
  if (existing) return json({error:'An account already exists with this mobile number. Please Sign In.'},409);

  const salt = randomHex(16);
  const hash = await hashCustomerPassword(password, salt);
  const result = await env.DB.prepare(`
    INSERT INTO customer_accounts
      (name,mobile,email,address,city,state,pincode,password_hash,password_salt,created_at,updated_at)
    VALUES (?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))
  `).bind(name,mobile,email,address,city,state,pincode,hash,salt).run();

  const accountId = result.meta?.last_row_id;
  const account = await env.DB.prepare(`SELECT * FROM customer_accounts WHERE id = ?`).bind(accountId).first();
  const token = await createCustomerSession(env, accountId);
  return json({ok:true, account:customerAccountView(account), token},201);
}
...
async function customerLogin(request, env) {
  ...
  const account = await env.DB.prepare(`SELECT * FROM customer_accounts WHERE mobile = ? LIMIT 1`).bind(mobile).first();
  if (!account) return json({error:'Account not found. Please Sign Up first.'},404);
  const hash = await hashCustomerPassword(password, account.password_salt);
  if (hash !== account.password_hash) return json({error:'Incorrect mobile or password'},401);

  const token = await createCustomerSession(env, account.id);
  return json({ok:true, account:customerAccountView(account), token});
}

async function customerUpdate(request, env) {
  ...
  await env.DB.prepare(`
    UPDATE customer_accounts SET name=?,email=?,address=?,city=?,state=?,pincode=?,updated_at=datetime('now') WHERE id=?
  `).bind(name,email,address,city,state,pincode,session.account.id).run();
  const account = await env.DB.prepare(`SELECT * FROM customer_accounts WHERE id=?`).bind(session.account.id).first();
  return json({ok:true,account:customerAccountView(account)});
}

async function customerOrders(request, env) {
  const session = await customerSession(request, env);
  if (!session) return json({error:'Please sign in again'},401);
  const mobile = session.account.mobile;
  const result = await env.DB.prepare(`
    SELECT id,order_number,customer_name,mobile,email,address,city,state,pincode,
           amount_paise,payment_method,payment_status,order_status,items_json,
           courier_name,tracking_number,tracking_url,shipped_at,delivered_at,created_at,updated_at
    FROM orders WHERE mobile = ? ORDER BY id DESC LIMIT 500
  `).bind(mobile).all();
